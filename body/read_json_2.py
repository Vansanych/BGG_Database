...
for name in range(60):
    with open(fr'../TeseraTop1000/{name}.json') as file:
        ...
        for r in range(len(g)):
            ...
            if len(f_n_r) > 1:
                first_name_rus.append([f_n_r[0], f_n_r[1]])
            else:
                first_name_rus.append(f_n_r[0])
            try:
                f_n_e = g[r]['title2'].split()
                f_n_e[0] = re.sub(r'\W', '', f_n_e[0])
                if len(f_n_r) > 1:
                    first_name_eng.append([f_n_e[0], f_n_e[1]])
                else:
                    first_name_eng.append(f_n_e[0])
...
with open(fr'../TopList/two_first_name_rus.json', 'w+') as file:
    file.write(json.dumps(first_name_rus, indent=4))

with open(fr'../TopList/two_first_name_eng.json', 'w+') as file:
